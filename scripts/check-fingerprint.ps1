...
# TT3.exe on disk and compare it against every build profile in
...
if (-not $ExePath) {
    ...
    $gamePaths = @(Find-AllGamePaths -GameId 'tt-isle-of-man-ride-on-the-edge-3')
    if ($gamePaths.Count -eq 0) { throw "TT Isle of Man: Ride on the Edge 3 not found. Pass -ExePath explicitly." }
    # Every store copy is its own build, so each installed EXE is checked.
    foreach ($gamePath in $gamePaths) {
        & $PSCommandPath -ExePath (Join-Path $gamePath 'TT3.exe')
        Write-Host ""
    }
    return
}
...
# Only the headers are read: the fingerprint sits in the first few hundred bytes
# and the EXE is tens of megabytes.
function Read-FilePrefix([string]$Path, [long]$Count) {
    $stream = [System.IO.File]::OpenRead($Path)
    try {
        $buffer = New-Object byte[] ([Math]::Min($Count, $stream.Length))
        $read = 0
...
$bytes = Read-FilePrefix $ExePath 0x40
...
if ($peOffset -ge 0) {
    # The PE signature, the COFF header and the optional header up to CheckSum.
    $bytes = Read-FilePrefix $ExePath ([long]$peOffset + 4 + 20 + 68)
}
...
if (-not $matched) {
    ...
    Write-Host "set renderer_do_update_rva to 0: IsProfileComplete() refuses a profile with a"
    Write-Host "zero in any of its RVAs (src/builds/build_profile.h), so the build then routes"
    Write-Host "to a profile that stays dormant."
}
