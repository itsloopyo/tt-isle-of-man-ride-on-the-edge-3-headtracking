...
- Fetched at: 2026-09-18T15:23:37.2947683+01:00
- Source: github
...
upstream name: TT3.exe statically imports VERSION.dll, and it is not a
KnownDLL, so a game-local copy wins the DLL search order. TT3.exe also
imports DINPUT8.dll, but that one carries the game's input API, so
VERSION.dll is the proxy.
